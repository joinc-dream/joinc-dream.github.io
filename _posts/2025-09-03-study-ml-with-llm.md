---
layout: single
title: "AI를 이용한 바이브 스터디 경험을 공유합니다"
...
## 바이브 스터디를 해보면 어떨까 ?
PHP 에서 시작해서 C, C++, Python, Ruby, Javascript, GoLang 등 다양한 언어를 이용해서 개발을 해왔습니다. 분야 역시 다양해서 웹 프로그래밍, 시스템/네트워크 프로그래밍, 검색엔진, 블록체인, 빅데이터, 데이터베이스, 클라우드, DevOps 최근에는 생성형-AI 까지 이런 저런 일들을 해왔습니다.

AI 쪽에서 일을 하다보니, 데이터와 함께 자연스럽게 ML(머신러닝)쪽도 관심을 가지게 되었습니다. 하지만 관심만 가지고 있었을 뿐 ML 쪽을 직접 할 생각을 하지는 못했습니다. 왜냐하면 **AI/ML**이 묶여있기는 하지만 생성형-AI 영역에서 데이터 처리가 중요하지 ML을 직접적으로 해야 하는 경우는 흔치 않기 때문입니다. 무엇보다 **어디에서 부터 공부해야 할지, 지금 한다고 해서 실무에서 써먹을 만큼의 지식을 얻을 수 있을지**가 걱정이 되었습니다.  

하지만 팀을 리딩하다보니 자연스럽게(어쩔 수 없이) ML 쪽을 봐야 하는 상황이 됐습니다. 그 동안은 ML 쪽은 전문가에게 맡기고 저는 귀동냥 정도로 대응을 했었는데 이번에 스터디를 해보기로 했습니다. 

이미 바이브코딩을 이용해서 성공적으로 프론트앤드 애플리케이션을 개발한 경험을 살려서 **바이브 스터디**를 해보기로 했습니다. 

## 바이브 스터디란
스터디는 간단한게 아닙니다. 동기부여, 왜 하는가, 무엇을 할 것인가, 어떻게 할 것인가를 알고 있어야 합니다. 개발관련 스터디라면 프레임워크, 툴, 개발 패턴, 데이터, 단계별 예제 코드 등 수많은 것들이 필요합니다. 혼자서는 이러한 것들을 준비하기 어렵기 때문에 책, 온라인 학습 플랫폼, 유튜브 등을 이용합니다. 운이 좋다면 멘토의 도움을 받을 수도 있겠죠.  하지만 어떤 것을 선택하든지 공간과 시간의 제약, 맞춤형 학습의 어려움이 존재합니다. 

**바이브 스터디**란 학습계획에서 학습완료까지의 전체 과정을 "생성형-AI"의 도움을 받아서 수행하는 것입니다. 정식으로 그렇게 부르지는 않지만 이렇게 부르기로 하겠습니다.

바이브 스터디는 **AI의 도움을 받아 개인 중심으로 공부하는 학습방법** 입니다. 전통적인 스터디가 모임 중심이라면, 바이브 스터디는 **AI를 학습 파트너로 삼아서** 혼자서도 충분히 몰입할 수 있는 교육 환경을 만드는 것이 특징입니다. 

바이브 스터디의 특징은 아래와 같습니다.
  1. 개인 맞춤 학습
    * AI가 개인의 학습 수준, 관심사, 속도에 맞춰서 커리큘럼, 자료와 설명을 제공합니다.
    * 교재나 강의에 구애받지 않고 **자기 주도적**으로 학습이 가능 합니다.
  1. 몰입감
    * 자기 주도적이기 때문에 스터디 환경만 잘 구성하면 자기만의 분위기 속에서 학습 할 수 있습니다.
  1. 즉각적인 피드백
    * 궁금한 점을 바로 물어보고 즉시 답변 할 수 있습니다.
    * 개발자의 경우 **Hands on** 까지 할 수 있으므로 현장 실습형 학습이 가능합니다.
  1. 다양한 학습 스타일 지원
    * 텍스트, 코드, 다이어그램, 요약 노트 등 다양한 방식을 지원합니다.
  1. 시간.장소의 제약 없음
    * 언제 어디서든 AI와 함께 스터디 할 수 있습니다.
    * 오프라인 모임이나 일정 조율이 필요 없습니다.

## 바이브 스터디 툴
제가 선택한 바이브 스터디 툴은 **Gemini CLI** 입니다. Gemini CLI는 구글에서 개발한 **오픈소스 AI 에이전트**로 터미널에서 자연어로 코드 작성, 디버깅, 자동화 등의 작업을 수행 할 수 있습니다. 코드 어시스턴트 목적으로 최적화된 툴이기는 하지만 다른 목적으로 사용해도 문제가 없습니다. 
...
## 스터디 계획 작성 
스터디를 시작하기 전에 Gemini CLI에게 **작업 지시**를 먼저해야 합니다. 프롬프트를 입력하는 방법도 있습니다만 system instruction 형태로 작성을 해야 AI 어시스턴트에게 지시사항을 전달하기가 좋습니다. **GEMINI.md** 파일에 지시사항을 저장하면 gemini 가 실행될 때, 파일을 읽어서 메모리에 저장하게 됩니다. 
...
## 스터디 계획 수립
ML 학습을 하려는 이유는 최근 **이커머스 판매 제품에 대한 수요예측**과 관련된 요구사항이 있었기 때문이다. 그래서 특정 판매제품에 대한 수요 예측 시나리오를 기반으로 스티터디 계획을 수립하기로 했습니다.
...
```
AI가 가상 시나리오 뿐만 아니라 pandas와 numpy 라이브러리를 사용하여, 가상 시나리오를 위한 테스트 예제도 만들어줄 것을 제안했습니다. 저는 pandas와 numpy는 잘 모르기 때문에 Gemini를 통해서 함게 조사하며 스터디를 진행했습니다.
...
```
gemini는 지금도 좋은 의견이기는 하지만, 일단은 단순한 시나리오로 ML 학습에 집중하는게 좋다는 의견을 제안했다. 충분히 좋은 제안이었기 때문에 여기에 따르기로 했다.

지금까지 AI의 응답에서 다음과 같은 패턴을 찾을 수 있었습니다. 

**AI는 칭찬하고 싶어한다**: Gemini, GPT를 비롯한 모든 언어모델에서 공통적으로 나타나는 현상인데, 중립적으로 정보를 전달하기 보다는 지나치게 낙관적이고 긍정적으로 답변을 합니다. 이러한 느낌은 저만 받고 있는게 아닙니다. 마케터로 시장조사를 하는 친구의 경우에도 어떤 서비스나 제품에 대한 시장 전망을 요청할 때, 너무 긍정적으로 답을 하는 경향이 있어서 주의해서 응답을 조절합니다.  

언어 모델은 개발 단계에서 사용자에게 부정적인 영향을 미치는 답변을 피하도록 설계되어 있습니다. 원래 목적은 극단적이거나 폭력적인 내용, 비관적이거나 절망적인 메시지, 사회적 논란을 야기할 수 있는 민감한 주제에 대해서는 직접적으로 부정적인 의견을 제시하지 않도록 학습이 됩니다. 그리고 언어모델이 학습에 사용하는 인터넷의 텍스트 역시 긍정적이거나 희망적인 내용, 문제 해결을 위한 조언과 같은 정보가 풍부하게 포함되어 있습니다. 이러다 보니 **긍정적으로 포장된 정보**에 (언어모델이 의도한 것은 아니지만)가스라이팅 될 수 있습니다. 따라서 정확한 정보 탐색이 목적인 경우에는 아래와 같이하면 더 효과적으로 대화할 수 있습니다.
...
다음 단계를 진행해 달라고 했습니다. 시나리오 데이터를 만들 것이라고 예상을 했으며, 실제 그렇게 제안했습니다. 
```
...
지금까지 AI와 작업했던 내용들이 매우 마음에 들었기 때문에 이들 정보를 파일로 저장해 달라고 요청 했습니다. 그 후 다음 단계를 진행하기로 했습니다. EDA를 진행하겠죠.!
...
✦ 네, EDA는 머신러닝 프로젝트의 매우 중요한 초기 단계입니다. 자세히 설명해
  ...
  다지는 과정이라고 할 수 있습니다.
```
무엇을 하는지 확실히 알게 됐다. 이제 EDA를 하기 위해서어떤 프레임워크 혹은 툴을 사용해야 할지 궁금해서 이에 대해서 알려달라고 요청했다.
...
---
  ...
  중 하나입니다.
```
데이터 분석과정에서 발생할 수 있는 여러 예외 사항들을 확인하고 이들을 어떻게 처리해야 할지에 대한 아이디어를 얻을 수 있었습니다.
...
중간 값보다 평균 값이 높다는 것은 "잘 팔린 제품이 있다"는 의미가 될 겁니다. 이는 긍정적인지를 물어봤습니다. 
```
✦ 흥미로운 질문입니다. 반드시 그렇지는 않습니다.
 ...
 > 다른 여러가지 변수가 있겠습니다만, 데이터만 놓고 본다면 프로모션이 효과적이지 않았다고 볼 수 있지 않나요 ? 왜냐하면 프로모션이
   성공적이었다면 평균 값이 올라갔을 테니까요.

✦ 아주 예리한 지적이고, 그렇게 생각하실 수 있습니다. '프로모션이 성공했다면 전체 평균이 높아야 하지 않을까?' 하는 논리적인 추론입니다.

  하지만, 전체 데이터의 평균(83.7)만으로는 프로모션의 효과를 아직 판단할 수 없습니다.

  ---

  이유: 평균은 모든 상황이 뒤섞인 값이다

  우리가 본 '전체 평균 83.7'이라는 값은 다음과 같은 모든 상황이 뒤섞여서 나온 결과입니다.

   * 프로모션을 한 날
   * 프로모션을 하지 않은 날
   * 가격이 비쌌던 날 vs 쌌던 날
   * 주말 vs 주중
   * 제품 B의 인기가 없던 초기 시절 vs 인기가 많아진 후

  프로모션이 매우 성공적이어서 판매량을 50%나 끌어올렸다고 해도, 프로모션을 하지 않은 나머지 85%의 날들, 특히 판매량이 저조했던 날들의
  영향이 훨씬 크기 때문에 전체 평균은 여전히 낮게 보일 수 있습니다.

  올바른 효과 측정 방법: 그룹별 비교

  프로모션의 효과를 제대로 측정하려면, 전체를 하나로 보는 것이 아니라 두 그룹으로 나누어 비교해야 합니다.

  > "프로모션을 한 날들의 평균 판매량" vs "프로모션을 하지 않은 날들의 평균 판매량"

  예를 들어, 우리가 데이터를 분석해보니 아래와 같은 결과가 나왔다고 가정해 봅시다.

   * 프로모션을 한 날의 평균 판매량: 120개
   * 프로모션을 하지 않은 날의 평균 판매량: 75개

  이런 결과가 나온다면, 비록 전체 평균이 83.7일지라도 "프로모션은 판매량을 평균 45개나 끌어올리는 매우 효과적인 전략이었다"라고 결론
  내릴 수 있습니다.

  ---

  결론:
  훌륭한 가설을 세워주셨습니다. 다만, 그 가설을 검증하기 위해서는 전체 평균이 아닌, 두 그룹(프로모션 O/X)을 나누어 비교하는 분석이
  필요합니다.

  이 비교 분석은 우리가 다음에 진행할 데이터 시각화(Data Visualization)나 그룹별 분석(Group-by Analysis)을 통해 명확하게 확인할 수
  있습니다. 예를 들어, 프로모션 여부에 따른 판매량 분포를 박스 플롯(Box Plot)으로 그려보면 그 차이를 한눈에 알 수 있게 됩니다.
```
...
## AI로 인한 개인과 조직의 업무 역할 변화
#### AI의 목표는 완전자동화된 시스템이 아니다